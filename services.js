...
/* ── PAGE HERO SLIDESHOW (lazy non-active slides) ── */
...
/* ── STUDIO SLIDESHOWS (with lazy background-image loading) ── */
(function initStudioSlideshows() {
  // Swap data-bg → background-image only when the band enters the viewport
  // This prevents loading 10 hero images on page load for slow connections
  ...
  const bandObserver = new IntersectionObserver((entries) => {
    entries.forEach(entry => {
      ...
      if (slides.length < 2) { bandObserver.unobserve(band); return; }

      const idx = Array.from(document.querySelectorAll('.studio-hero-band'))
                        .indexOf(band);
      let cur = 0;
      setTimeout(() => {
        setInterval(() => {
...
/* ── PRISM PROGRESS INDICATOR (services page — 5 divisions) ── */
(function initPrismProgress() {
  ...
  const divisionSections = {
    ...
    motion:   document.querySelectorAll('[data-div="motion"]'),
  };
...
/* ── SCROLL REVEAL (closing CTA section) ── */
(function initReveal() {
...
const WA_DEFAULT = '2347053815985';
const WA_WEB_MOTION = '2347042776167';

function waLink(msg, tagClass) {
  const number = tagClass === 'web-tag' || tagClass === 'motion-tag' ? WA_WEB_MOTION : WA_DEFAULT;
...
/* Maps every (studio, card) pair used in the markup's data-studio/data-card
   attributes (and matching openOverlay('studio','card') onclick calls) to
   the correct key in overlayData. This is the single source of truth the
   enquiry buttons resolve against — every button on the Services page must
   have an entry here or it will not become clickable. */
const SERVICE_KEY_MAP = {
  'media-photography':   'media-photo',
  'media-videography':   'media-photo',
  'media-graphics':      'media-graphics',
  'media-content':       'media-content',
  'media-livestream':    'media-content',

  'entertainment-corporate':  'events-planning',
  'entertainment-concerts':   'events-production',
  'entertainment-activation': 'events-planning',

  'talent-representation': 'talent-rep',
  'talent-brand-dev':      'talent-rep',
  'talent-brand-deals':    'talent-deals',
  'talent-crisis':         'talent-rep',

  'business-brand-strategy': 'biz-brand',
  'business-pr':             'biz-marketing',
  'business-marketing':      'biz-marketing',

  'web-websites':    'web-websites',
  'web-ecommerce':   'web-websites',
  'web-mobile-apps': 'web-apps',
  'web-ai':          'web-ai',

  'motion-motion-graphics': 'motion-content',
  'motion-2d-animation':    'motion-explainer',
  'motion-3d':               'motion-brand',
};

function getServiceKeyFromButton(btn) {
  let studio, dataCard;

  const onclick = btn.getAttribute('onclick');
  if (onclick) {
    const match = onclick.match(/openOverlay\('([^']+)','([^']+)'\)/);
    if (match) { studio = match[1]; dataCard = match[2]; }
  }
  if (!studio) {
    const card = btn.closest('.svc-conv-card');
    if (card) { studio = card.dataset.studio; dataCard = card.dataset.card; }
  }
  if (!studio || !dataCard) return null;

  const rawKey = `${studio}-${dataCard}`;
  return SERVICE_KEY_MAP[rawKey] || rawKey;
}

/* Fallback so a button never dies with a "openOverlay is not defined" error
   if convertServiceEnquireButtons somehow runs before this button is
   converted (e.g. a future card gets added without a matching data-studio/
   data-card, or a stray click fires before DOM processing finishes). Sends
   the person straight to the correct WhatsApp number for that studio. */
function openOverlay(studio, dataCard) {
  const key = SERVICE_KEY_MAP[`${studio}-${dataCard}`] || `${studio}-${dataCard}`;
  const d = overlayData[key];
  const href = d ? waLink(waMsg(d.title), d.tagClass) : waLink(waMsg('your project'), '');
  window.open(href, '_blank', 'noopener');
}

function convertServiceEnquireButtons() {
  document.querySelectorAll('.scc-expand-btn').forEach(btn => {
    if (!(btn instanceof HTMLElement)) return;
    const key = getServiceKeyFromButton(btn);
    if (!key || !overlayData[key]) return;
    const d = overlayData[key];
    const href = waLink(waMsg(d.title), d.tagClass);
    const anchor = document.createElement('a');
    anchor.className = btn.className;
    anchor.href = href;
    anchor.target = '_blank';
    anchor.rel = 'noopener';
    anchor.innerHTML = btn.innerHTML;
...
function buildOverlay(key) {
  ...
  const bentoItems = d.items.map(item => `
    <div class="bento-item ${d.bentoClass}">
      ...
      <a href="${waLink(item.wa, d.tagClass)}" class="bento-wa-btn ${d.waClass}" target="_blank" rel="noopener">
        ${waIcon} Get Started
      </a>
    </div>
  `).join('');

  const notSureWa = waLink(waMsg(`${d.title} (not sure which option fits)`), d.tagClass);
